...
public function gals(){
	...
	if( empty($cid) && empty($pid) )
	{
		...
		{
			$result_array[] = "<a href='?test.php?cid=".$row[0]."'>".$row[1]."</a> "."(".$row[2].")";
		}
	...
	else if( $cid && empty( $pid ) )
	{
		...
		else
		{
			while( $row = mysql_fetch_array( $result ) )
			{
				$result_array[] = "<a href='?test.php?cid=$cid&pid=".$row[0]."'><img src='".$images_dir."/tb_".$row[2]."' border='0' alt='".$row[1]."' /></a>";
			}
			mysql_free_result( $result );	
	...
	else if( $pid )
	{
		...
		else
		{
			...
			$result_final .= "<tr>\n\t<td>
						<a href='?'>Categories</a> &gt; 
						<a href='?cid=$cid'>$category_name</a></td>\n</tr>\n";
...
// de som skickas till vyn buy
return $ret="

<html>
<head>
<link media='screen' href='stylesheet.css' rel='stylesheet'>
